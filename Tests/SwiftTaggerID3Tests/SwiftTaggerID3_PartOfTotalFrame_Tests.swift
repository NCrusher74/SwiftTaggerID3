...
class SwiftTaggerID3_PartOfTotalFrame_Tests: XCTestCase {
    ...
    func testSetToNil() throws {
        ...
        XCTAssertNil(tag?.listMetadata().first(where: {$0.frameKey == .discNumber}))
        XCTAssertNil(tag?.listMetadata().first(where: {$0.frameKey == .trackNumber}))
        
        let outputUrl = try localDirectory(fileName: "partOfTotalRemovaltest", fileExtension: "mp3")
        XCTAssertNoThrow(try TestFile.v24.mp3File()?.write(tagVersion: .v2_4,
        ...
        XCTAssertNil(output.listMetadata().first(where: {$0.frameKey == .discNumber}))
        XCTAssertNil(output.listMetadata().first(where: {$0.frameKey == .trackNumber}))
    }
}
